...
nframes=0;
  ...
  for  eb_N0_db=0:8;
    ...
    nframes=0;
    total_size=0;
  while erros<80 %|| frame_size<4*log2(M).*10e4
...
codedbits=codifica_hamming(mbits,G);
...
%% Demodulador
[Zbits]=demodulador_qpsk(codedbits, ps, Ifilter_n, Qfilter_n, OS, delay, M);

[Zbits_un]=demodulador_qpsk(mbits, ps, Ifilter_n_un, Qfilter_n_un, OS, delay, M);
% 
% figure; stem(m(1:40));
% xlabel('n','FontSize',11); ylabel('Q','FontSize',11);
% grid on;
...
%% contagem de erros
erros=sum(abs(sign(decodedbits-mbits)))
erros_un=sum(abs(sign(Zbits_un-mbits)))
...
BER_un(index)=erros_un/(frame_size);%*nframes);%.*nframes);
