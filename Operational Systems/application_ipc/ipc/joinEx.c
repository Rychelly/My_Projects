...
int main(int argc, char ** argv) { 
  ...
  pthread_t thrd[NTHRDS]; //data structure for managing several worker thread instances
  pthread_attr_t attr; //data structure for managing thread attributes
  pthread_attr_init(&attr); //initialise thread attribute data ...
  pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE); //...and set "joinable" attribute 
